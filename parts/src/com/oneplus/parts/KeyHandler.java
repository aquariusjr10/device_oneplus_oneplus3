...
import com.oneplus.parts.utils.Keycodes;
...
public class KeyHandler implements DeviceKeyHandler {
    ...
    private class EventHandler extends Handler {
        ...
        public void handleMessage(Message message) {
            switch (message.arg1) {
                case Keycodes.KEY_TGESTURES_DOUBLE_TAP:
                case Keycodes.KEY_TGESTURES_CIRCLE:
                    ...
                    doHapticFeedback();
                    break;
                case Keycodes.KEY_TGESTURES_TWO_SWIPE:
                    dispatchMediaKeyWithWakeLockToMediaSession(KeyEvent.KEYCODE_MEDIA_PLAY_PAUSE);

                    doHapticFeedback();
                    break;
                case Keycodes.KEY_TGESTURES_DOWN_ARROW:
                    ...
                    break;
                case Keycodes.KEY_TGESTURES_LEFT_V:
                    dispatchMediaKeyWithWakeLockToMediaSession(KeyEvent.KEYCODE_MEDIA_PREVIOUS);

                    doHapticFeedback();
                    break;
                case Keycodes.KEY_TGESTURES_RIGHT_V:
    ...
    public KeyEvent handleKeyEvent(KeyEvent event) {
        ...
        boolean isKeySupported = ArrayUtils.contains(Keycodes.KEYS_TGESTURES, scanCode);
        boolean isSliderModeSupported = Keycodes.MODES_SLIDER.indexOfKey(scanCode) >= 0;
        if (!isKeySupported && !isSliderModeSupported) {
            return event;
        }

        // We only want ACTION_UP event, except KEY_DOUBLE_TAP
        if (scanCode == Keycodes.KEY_TGESTURES_DOUBLE_TAP) {
        ...
        if (isSliderModeSupported) {
            if (scanCode == Keycodes.MODE_SLIDER_VIBRATION) {
                mNotificationManager.setZenMode(Keycodes.ZEN_MODE_OFF, null, TAG);
                mAudioManager.setRingerModeInternal(AudioManager.RINGER_MODE_VIBRATE);
            } else {
                mAudioManager.setRingerModeInternal(AudioManager.RINGER_MODE_NORMAL);
                mNotificationManager.setZenMode(Keycodes.MODES_SLIDER.get(scanCode), null, TAG);
            }
    ...
    private void doHapticFeedback() {
        ...
        boolean enabled = (boolean) Preferences.getPreferenceValue(mContext.getApplicationContext(), Preferences.KEY_TGESTURES_HAPTIC_FEEDBACK);
        if (enabled) {
